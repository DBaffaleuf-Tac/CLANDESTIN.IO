...
1) For data privacy reasons, you should not pass any sensitive data in a prompt to an external LLM service / model. Use local model instread (eg Ollama)
2) Clandestinio is a python command-line tool to help pseudonymize data **in your test/dev/UAT environments**, on a per-table basis. It is based on generative AI to help identify which columns in the table can be considered sensitive as per the GDPR regulations, and eventually substitute data while preserving the semantic. 
...
It seems the number of seconds per iteration only fluctuates between 6 and 21 seconds depending on the traffic on the API endpoints, which makes the performance quite consistent across batches :
